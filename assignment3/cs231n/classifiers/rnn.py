...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    if self.cell_type == 'rnn':
        out, cache = rnn_forward(embedding_out, h0, Wx, Wh, b)

    elif self.cell_type == 'lstm':
        out, cache = lstm_forward(embedding_out, h0, Wx, Wh, b)

    # (4)
    scores, scores_cache = temporal_affine_forward(out, W_vocab, b_vocab)
    ...
    # BACKWARD PASS
    # (4)
    dx, dW_vocab, db_vocab = temporal_affine_backward(dout, scores_cache)
  ...
  def sample(self, features, max_length=30):
    ...
    c = np.zeros_like(h)
    words = np.array([W_embed[self._start]] * N)
    for timestep in range(0, max_length):
        # (2)
        if self.cell_type == 'rnn':
            next_h, cache = rnn_step_forward(words, h, Wx, Wh, b)
        elif self.cell_type == 'lstm':
            next_h, next_c, cache = lstm_step_forward(words, h, c, Wx, Wh, b)
            # raise ValueError("Not implemented")
        # (3)
        scores = np.dot(next_h, W_vocab) + b_vocab
        # (4) (1)
        word_indices = np.argmax(scores, axis=1)
        captions[:, timestep] = word_indices
        # (1)
        h = next_h
        if self.cell_type == 'lstm':
            c = next_c
        words = W_embed[word_indices]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    return captions
