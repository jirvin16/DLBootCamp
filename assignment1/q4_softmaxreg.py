...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    # - labels: labels corresponding to the feature vectors  # (H, )       
    # - weights: weights of the regressor  (D, H)                         
    ...
    x = np.zeros_like(prob)
    x[range(N), labels] = 1
    grad = np.dot(features.T, prob - x) / N + regularization * weights # (D, H)
